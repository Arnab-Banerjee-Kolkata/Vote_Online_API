...
if($stmt3->fetch() && $postAuthKey1==$postAuthKey2)
{
    ...
    if($category=="voter")          //Called from RPD
    {
        ...
        {            
            ...
            $stmt=$conn->prepare("SELECT otp FROM Booth WHERE booth_id=?");
    ...
    {
        $stmt=$conn->prepare("SELECT COUNT(aadhaar_no) FROM Govt_Approval WHERE election_id=? AND aadhaar_no=?");
        $stmt->bind_param("ss",$electionId, $aadhaarNo);
        $stmt->execute();
        ...
        $stmt->execute();
        $stmt->bind_result($otp2);
        if($stmt->fetch() && $otp1==$otp2 && $response['validApproval'])
        {                     
            $response['success']=true;             
        }
        $stmt->close();
